...
public class QuestionDetail {
    ...
    public List<QuestionDetail> subQuestion = new ArrayList<>();
    ...
    public QuestionDetail(Map<?, ?> m) {
        ...
        Object o = m.get("id");
        this.id = o == null ? null : o.toString();
        Object t = m.get("type");
        this.type = t == null ? null : t.toString();
        Object tn = m.get("typeName");
        this.typeName = tn == null ? null : tn.toString();
        ...
        if (cat instanceof List) {
            for (Object c : (List<?>) cat) {
                if (c != null) category.add(c.toString());
            }
        } else if (cat != null) {
            category.add(cat.toString());
        }

        Object a = m.get("analysis");
        this.analysis = a == null ? null : a.toString();
        Object ans = m.get("answers");
        this.answers = ans == null ? null : ans.toString();
        ...
        this.difficulty = diff == null ? null : diff.toString();
        Object diffN = m.get("difficultyName");
        this.difficultyName = diffN == null ? null : diffN.toString();

        Object lbl = m.get("label");
        if (lbl instanceof List) {
            for (Object l : (List<?>) lbl) {
                if (l != null) {
        ...
        if (examedObj instanceof Boolean) {
            this.examed = (Boolean) examedObj;
        } else {
            this.examed = false;
        }
        Object boutiqueObj = m.get("boutique");
        if (boutiqueObj instanceof Boolean) {
            this.boutique = (Boolean) boutiqueObj;
        } else {
            this.boutique = false;
        }
        Object examableObj = m.get("examable");
        if (examableObj instanceof Boolean) {
            this.examable = (Boolean) examableObj;
        } else {
            this.examable = false;
        }
        Object exerciseObj = m.get("exercise");
        if (exerciseObj instanceof Boolean) {
            this.exercise = (Boolean) exerciseObj;
        } else {
            this.exercise = false;
        }
        Object langObj = m.get("languages");
        if (langObj instanceof Boolean) {
            this.languages = (Boolean) langObj;
        } else {
        ...
        Object ak = m.get("answerKey");
        if (ak instanceof Map) this.answerKey = (Map<?, ?>) ak;
        ...
        Object qsNames = m.get("qsCategoriesName");
        if (qsNames instanceof List) this.qsCategoriesName.addAll((List<?>) qsNames);

        Object bid = m.get("bankId");
        this.bankId = bid == null ? null : bid.toString();
        Object qbid = m.get("questionBankId");
        this.questionBankId = qbid == null ? null : qbid.toString();
        Object ut = m.get("useType");
        this.useType = ut == null ? null : ut.toString();

        Object dn = m.get("difficultyNumber");
        if (dn instanceof Number) this.difficultyNumber = ((Number) dn).doubleValue();
        else if (dn != null) {
            try {
                this.difficultyNumber = Double.parseDouble(dn.toString());
            } catch (Exception ignored) {
            }
        }

        Object mk = m.get("mark");
        this.mark = mk == null ? null : mk.toString();
        this.cateids = m.get("cateids");

        Object ifb = m.get("isFeedback");
        if (ifb instanceof Number) {
            this.isFeedback = ((Number) ifb).intValue();
        }
        Object hec = m.get("handledErrorCount");
        if (hec instanceof Number) {
            this.handledErrorCount = ((Number) hec).intValue();
        }
        Object aec = m.get("allErrorCount");
        if (aec instanceof Number) {
            this.allErrorCount = ((Number) aec).intValue();
        }

        Object subs = m.get("subQuestion");
        if (subs instanceof List) {
            for (Object s : (List<?>) subs) {
                if (s instanceof Map) {
                    this.subQuestion.add(new QuestionDetail((Map<?, ?>) s));
                } else if (s instanceof QuestionDetail) {
                    this.subQuestion.add((QuestionDetail) s);
                }
            }
        }
        Object ap = m.get("answerPoints");
        if (ap instanceof List) {
            this.answerPoints.addAll((List<?>) ap);
        }
    }
    ...
    public static class Option {
        ...
        public Option(Map<?, ?> m) {
            ...
            if (seq instanceof Number) {
                this.answerSeq = ((Number) seq).intValue();
            } else if (seq != null) {
                try {
                    this.answerSeq = Integer.parseInt(seq.toString());
                } catch (Exception ignored) {
                }
            }
            ...
            if (ord instanceof Number) {
                this.orders = ((Number) ord).intValue();
            } else if (ord != null) {
                try {
                    this.orders = Integer.parseInt(ord.toString());
                } catch (Exception ignored) {
                }
            }
        }
    ...
    public static class AnswerObj {
        ...
        public AnswerObj(Map<?, ?> m) {
            ...
            if (ans instanceof Number) {
                this.answer = ((Number) ans).intValue();
            } else if (ans != null) {
                try {
                    this.answer = Integer.parseInt(ans.toString());
                } catch (Exception ignored) {
                }
            }
            Object ord = m.get("orders");
